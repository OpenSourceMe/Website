...
This is where *isomorphic rendering* comes in. Isomorphic applications are applications that run both on the client side and on the server. This is useful, as what it means is that when a user visits a non-default route, we can render the application with on the server, and simulate some actions to advance it to a non-default state. Specifically in the case of this engine, we can saturate the state of the application with non-default routing information. When the user visits a non-default route, e.g. www.lachlankermode.com/about, we can send them our application with a state that corresponds to what they expect; the application at its *about* route.
...
Docker is a software for containerization that provides a lot of the same benefits that virtual machines do. When you run an application in a virtual machine, you don't have to deal with different configurations across different machines with different specifications, as all of those requirements and configuration is encapsulated within the virtual machine. Docker is a complex and multi-faceted beast, but in the context of this application it provides one very simple service: the ability to run the application with nothing but Docker installed. This is because Docker allows you to create things called *images* that package all of the required installations and code, which can then be spun up into things called *containers*: self-contained processes that can run the application code, which we can then reference in order to actually use the application. At the moment, Docker runs natively on Linux. You can also run it on Mac and Windows, but you have to jump through a couple of extra hoops. [Read installation docs here.](https://docs.docker.com/engine/installation/)
